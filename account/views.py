...
from account.serializers import RegisterSerializer, LoginSerializer, CreateNewPasswordSerializer, FollowSerializer, UserSerializer, SearchSerializer
...
class RegistrationView(APIView):
    def post(self, request):
        ...
        if serializer.is_valid(raise_exception=True):
            serializer.save()
            return Response('Successfuly registered!', status=status.HTTP_201_CREATED)
...
class ActivationView(APIView):
    def get(self, request, activation_code):
        ...
        user.save()
        return Response('Account successfuly activated!', status=status.HTTP_200_OK)
...
class FollowUserView(APIView):
    ...
    def get(self, request, format=None, username=None):
        ...
        if from_user != to_user:
            ...
            raise Exception('Do not cheat on yourself!')
